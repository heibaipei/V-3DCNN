...
conv_1_shape = '4*4*1*8'
pool_1_shape = 'None'

# conv_2_shape = 'None'
conv_2_shape = '4*4*1*16'
pool_2_shape = 'None'

# conv_3_shape = 'None'
conv_3_shape = '4*4*1*32'
pool_3_shape = 'None'
...
dataset_dir = "./deap_shuffled_data/multi_classification/"
###load training set
...
def apply_conv2d(x, filter_height, filter_width, in_channels, out_channels, kernel_stride,name):
    ...
    return tf.nn.selu(conv2d(x, weight, kernel_stride))
...
for curr_fold in range(fold):
    ...
    #                            'f1_score': test_f1})  # , 'roc_auc':test_auc})
        writer = pd.ExcelWriter(
            "./result/multi_classification/2D-3D/"+ data_file +"_"+str(curr_fold)+ ".xlsx")
