...
dataset_dir = "/home/hy01/emotion_first/deap_shuffled_data/origin_"+arousal_or_valence+"/"
###load training set
...
#print("one_hot_labels:", train_one_hot_labels.shape)
...
split = np.random.rand(len(cnn_datasets)) < 0.9
...
# readout layer
y_ = apply_readout(cnn_out_fuse, fc_size, n_labels,'readout')
...
#    os.system("mkdir -p ./result/cnn_rnn_parallel/tune_rnn_layer/" + output_dir)
    ...
    ins = pd.DataFrame({'conv_1': conv_1_shape, 'pool_1': pool_1_shape, 'conv_2': conv_2_shape, 'pool_2': pool_2_shape,
                        'conv_3': conv_3_shape, 'pool_3': pool_3_shape, 'conv_fuse': conv_fuse,
...
#                            'f1_score': test_f1})  # , 'roc_auc':test_auc})
    writer = pd.ExcelWriter(
        "./result/2D_origin/"+ data_file+"_"+arousal_or_valence+ ".xlsx")
    ...
    paralmeter_count = 0
		...
		for variable in tf.trainable_variables():
			...
			for dim in variable.get_shape():
				count = count * dim
			paralmeter_count = paralmeter_count + count
			# model_dict[variable.name]=session.run(variable)
		# sio.savemat(arousal_or_valence+"_model.mat",model_dict)

		print("-----------------------------------------------------------------")
		print("------------total parameters", paralmeter_count, "--------------------------")
    # for variable in tf.trainable_variables():
    #     print(variable.name,"->",variable.get_shape())
    '''
